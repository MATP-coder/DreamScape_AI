...
export default async function handler(req, res) {
  ...
  // Compose style into the prompt if provided. We build an expressive, viral prompt
  // that emphasises the dreamlike nature, rich detail and shareability. This ensures
  // the resulting artwork feels unique and highly appealing, giving real value
  // beyond simply prompting DALL‑E yourself. We truncate the user prompt to avoid
  // exceeding token limits. ArtStation and Behance references push the model to
  // generate polished digital art.
  const stylePrompt = style ? ` im Stil von ${style}` : ''
  const basePrompt = String(prompt).slice(0, 900)
  const finalPrompt =
    `Ein traumhaftes Kunstwerk, inspiriert von folgender Szene: ${basePrompt}${stylePrompt}. ` +
    'Hochdetailliert, ausgewogene Komposition, faszinierende Lichtstimmung, ' +
    'visuell beeindruckend, perfekt für virale Posts. Trending auf Instagram, Behance und ArtStation.'
